...
bool cpreferences_begin(cpreferences_t *prefs, const char *namespace_name,
                        bool read_only) {
...
size_t cpreferences_get_string(cpreferences_t *prefs, const char *key, char *buf,
  ...
  String value =
      prefs->prefs.getString(key, default_value != NULL ? default_value : "");
  return copy_string(value, buf, buf_size);
}

bool cpreferences_put_string(cpreferences_t *prefs, const char *key,
                             const char *value) {
...
bool cpreferences_get_bool(cpreferences_t *prefs, const char *key,
                           bool default_value) {
  if (prefs == NULL || !prefs->opened || key == NULL) {
...
uint8_t cpreferences_get_uchar(cpreferences_t *prefs, const char *key,
...
uint16_t cpreferences_get_ushort(cpreferences_t *prefs, const char *key,
...
bool cpreferences_put_ushort(cpreferences_t *prefs, const char *key,
                             uint16_t value) {
  if (prefs == NULL || !prefs->opened || key == NULL) {
    return false;
  }
...
uint32_t cpreferences_get_ulong(cpreferences_t *prefs, const char *key,
